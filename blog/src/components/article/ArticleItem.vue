<template>
  <vs-card type="1" style="margin-bottom: 30px">
    <template #title>
        <div class="title"><span v-if="article.artInfoIsTop == 1" class="body-2">[置顶]</span>{{ article.artInfoTitle }}</div>
        <div><i class="fa fa-clock-o"></i> {{article.artInfoCreatedTime}}</div>
    </template>
    <template #img v-if="article.coverUrl">
      <img :src="article.coverUrl + '/thumbnail800'" style="height: 100%;width: 100%"/>
    </template>
    <template #text>
    ...
    <template #interactions>
      <vs-button  shadow size="small" circle>
